package com.application.image.lib_ui.material_design.recylerview.slide;
 ...
 * on 2020/12/3
 * RecylerView侧滑出现删除按钮，适用在海关忘记密码时候，删除这个任务
 */
public class SlideRecyclerView extends RecyclerView {
    ...
    public boolean onInterceptTouchEvent(MotionEvent e) {
        ...
        int y = (int) e.getY();
        obtainVelocity(e);
        Log.e("slide","onInterceptTouchEvent enter =");
        switch (e.getAction()) {
            case MotionEvent.ACTION_DOWN:
                ...
                if (mPosition != INVALID_POSITION) {
                    ...
                    if (mFlingView.getChildCount() == 2) {
                        int xmlWidth = 0;
                        try {
                            xmlWidth = DensityUtil.dip2px(mContext,mFlingView.getChildAt(1).getWidth());
                            // 注意这里不需要改变xml布局的内容
                            int measureWidth = ((ViewGroup)((ViewGroup)mFlingView.getChildAt(0)).getChildAt(4)).
                                    getChildAt(0).getWidth();
                            mMenuViewWidth = measureWidth + DensityUtil.dip2px(mContext,20);
                        } catch (Exception exception) {
                            mMenuViewWidth = xmlWidth + DensityUtil.dip2px(mContext,20);
                        }
                        configDeleButton();
                    } else {
    ...
    private void configDeleButton() {
        LinearLayout.LayoutParams layoutParams = new LinearLayout.LayoutParams(mMenuViewWidth,DensityUtil.dip2px(mContext,36));
        layoutParams.leftMargin = DensityUtil.dip2px(mContext,20);
        layoutParams.gravity = Gravity.CENTER;
        mFlingView.getChildAt(1).setLayoutParams(layoutParams);
    }

    @Override
    public boolean onTouchEvent(MotionEvent e) {
        Log.e("slide","ACTION_MOVE dx mIsSlide =" + mIsSlide + "--mPosition = " + mPosition);
        if (mIsSlide && mPosition != INVALID_POSITION) {
            float x = e.getX();
            obtainVelocity(e);
            switch (e.getAction()) {
                case MotionEvent.ACTION_DOWN:   // 因为没有拦截，所以不会被调用到
                    Log.e("slide","ACTION_DOWN ");
                    break;
                case MotionEvent.ACTION_MOVE:
                    Log.e("slide","ACTION_MOVE ");
                    Log.e("slide","ACTION_MOVE mMenuViewWidth = " + mMenuViewWidth);
                    // 随手指滑动
                    if (mMenuViewWidth != INVALID_CHILD_WIDTH) {
                        float dx = mLastX - x;
                        if (mFlingView.getScrollX() + dx <= mMenuViewWidth
                                && mFlingView.getScrollX() + dx > 0) {
                            Log.e("slide","ACTION_MOVE dx =" + dx);
