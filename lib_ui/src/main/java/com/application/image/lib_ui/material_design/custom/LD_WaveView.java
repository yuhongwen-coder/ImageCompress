...
public class LD_WaveView extends View {
    ...
    private int mWaveColor2;//波的颜色2

    private Paint mPaint;//波浪轨迹画笔
    private Paint mPaint2;//波浪轨迹画笔2

    private Paint mCirclePaint;//圆环画笔

    private Paint mBorderPaint;//边界画笔

    private int mBorderWidth = 1;//边界宽度
    ...
    private List<Point> mPoints2;//点的集合
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (!isMeasure && Math.abs(getMeasuredHeight() - getMeasuredWidth()) < 50) {//只计算一次就够了 ，relativelayout的时候要绘制两次 加个宽高判断
            ...
            mCirclePaint.setStrokeWidth(mStrokeWidth+100);//线是有宽度的 采用了这种方式画圆环
            isMeasure = true;
        }
    }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        //创建圆形预览框
        Path path = new Path();
        path.addCircle(mViewWidth / 2, mViewHeight / 2, mViewHeight / 2, Path.Direction.CCW);
        canvas.save();
        canvas.clipPath(path);
        canvas.drawCircle(mViewWidth / 2, mViewHeight / 2, mViewHeight / 2, mBorderPaint);
        //绘制线条
        mPath.reset();
        int i = 0;
        ...
        mPath.moveTo(mPoints2.get(0).x + mLeftWaveMoveLength, mPoints2.get(0).y - mViewHeight * mProgress / 100);
        for (; i < mPoints2.size() - 2; i += 2) {
            mPath.quadTo(mPoints2.get(i + 1).x + mLeftWaveMoveLength, mPoints2.get(i + 1).y - mViewHeight * mProgress / 100, mPoints2.get(i + 2).x + mLeftWaveMoveLength, mPoints2.get(i + 2).y - mViewHeight * mProgress / 100);
        }
        mPath.lineTo(mPoints2.get(i).x + mLeftWaveMoveLength, mViewHeight);
        mPath.lineTo(mPoints2.get(0).x + mLeftWaveMoveLength, mViewHeight);
        mPath.close();
...
//            canvas.drawCircle(mViewWidth / 2, mViewHeight / 2, mViewHeight / 2, circlePaint);
            ...
            //绘制边界
//            mBorderPaint.setStrokeWidth(mBorderWidth / 2+8);
